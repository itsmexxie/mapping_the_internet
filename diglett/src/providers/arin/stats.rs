...
impl StatsProvider {
                ...
                if parsed_header {
                    ...
                    match parts[2] {
                        "ipv4" => {
                            ...
                            i +=
                                (ipv4_offset + (HEADER_SIZE - parsed_header_sections) + 1) as usize;
    ...
    pub async fn register_and_load(config: &Config, registered: &mut Vec<Vec<String>>) -> Self {
        registered.push(
            SECTIONS
                .clone()
                .iter()
        ...
        StatsProvider::load(config).await
    }
}
